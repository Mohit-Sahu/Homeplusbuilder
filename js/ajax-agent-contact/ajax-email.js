(function($) {
	...
	function sendContactInfo() {
		...
		contactFormXML.open("POST", "js/ajax-agent-contact/send_email.php",true);
		contactFormXML.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		var form = $('#pm-agent-contact-form');
	...
	contactFormRequestHandler=function(data){
		...
		if (contactFormXML.readyState==4 && contactFormXML.status==200)	{
			...
			if(responseTextObject.status == "first_name_error") {
				...
				$('#pm-contact-form-response-agent').html('Please provide your first name.');
				$('#pm_s_first_name').addClass('invalid_field');
				bindClickEvent();
				
			} else if(responseTextObject.status == "last_name_error") {
				
				$('#pm-contact-form-response-agent').html('Please provide your last name.');
				$('#pm_s_last_name').addClass('invalid_field');
				bindClickEvent();
				
			} else if(responseTextObject.status == "email_error") {
				
				$('#pm-contact-form-response-agent').html('Please provide a valid email address.');
				$('#pm_s_email_address').addClass('invalid_field');
				bindClickEvent();
				
			} else if(responseTextObject.status == "message_error") {
				
				$('#pm-contact-form-response-agent').html('Please provide a message for your inquiry.');
				$('#pm_s_message').addClass('invalid_field');
				bindClickEvent();
				
			/*} else if(responseTextObject.status == "security_error") {
				
				$('#pm-contact-form-response-agent').html('Invalid security code entered.');
				$('#security_code').addClass('invalid_field');
				bindClickEvent();*/
				
			} else if(responseTextObject.status == "success"){
				
				$('#pm-contact-form-response-agent').html('Your inquiry has been received, thank you.');
				$('#pm-agent-contact-form-btn').fadeOut();
				
			} else if(responseTextObject.status == "failed"){
				
				$('#pm-contact-form-response-agent').html('A system error occurred. Please try again later.');
				$('#pm-agent-contact-form-btn').fadeOut();
	...
	function bindClickEvent() {
		...
		$('#pm-agent-contact-form-btn').click(function(e) {
					
			sendContactInfo();
		
			$(this).unbind("click");
			
			$('#pm-contact-form-response-agent').html("Validating message, please wait...");
