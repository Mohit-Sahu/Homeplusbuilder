(function($) {
	...
	function sendContactInfo() {
		...
		contactFormXML.open("POST", "js/ajax-property-agent-contact/send_email.php",true);
		contactFormXML.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
		var form = $('#pm-agent-property-contact-form');
	...
	contactFormRequestHandler=function(data){
		...
		if (contactFormXML.readyState==4 && contactFormXML.status==200)	{
			...
			if(responseTextObject.status == "full_name_error") {
				
				$('#pm-contact-form-response').html('Please provide your full name.');
	...
	function bindClickEvent() {
		...
		$('#pm-agent-property-contact-form-btn').click(function(e) {
